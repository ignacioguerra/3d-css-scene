...
function frame() {
  ...
  for(let i = 0; i < mob.length; i++) {
    mob[i].style.transform = `translateX(${-1000+(i*2000/mob.length)}px) translateZ(${-1000 + Math.sin(radians(frameCount)+i)*500}px)  rotateY(${-cam.ay}deg) rotateX(${-cam.ax}deg) scale(0.1) translateY(80px) skewX(0) skewY(0)`
  }
  frameCount++
  move()
  update()
